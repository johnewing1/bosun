...
func TestQuery(t *testing.T) {
	...
	for _, test := range tests {
		...
		if err != test.err {
			t.Errorf("Query failed, expect error to be %v, got %v", err, test.err)
		}
...
func TestCacheKeyMatch(t *testing.T) {
	...
	var tests = []struct {
		...
		{req: Request{
			...
			Dimensions: [][]Dimension{{{Name: "InstanceId", Value: "i-0106b4d25c54baac7"}}},
			Profile:    "prod",
		},
...
func TestCacheKeyMisMatch(t *testing.T) {
	...
	var tests = []Request{
		...
		},
		{
			...
			Period:     "300",
			Statistic:  "Sum",
			Dimensions: [][]Dimension{{{Name: "InstanceId", Value: "i-01064646d6d6baac7"}}},
			Profile:    "prod",
		},
		{
			Start:      &start,
